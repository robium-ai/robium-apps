...
- **Real compute:** Linux/amd64, NVIDIA CUDA, first feasibility attempt on one
  A40 or RTX A6000 48 GB; one A100 80 GB rerun is allowed only if VRAM blocks
  the 48 GB attempt.
...
| Live compute | RunPod Secure Cloud Pod | exact A40/A6000 allowlist; optional A100 feasibility fallback | Required 48 GB NVIDIA class with per-visitor isolation. Smaller silent fallbacks and Cloud Run CPU inference are rejected. |
...
| --- | --- | --- |
| Checkpoint may exceed 48 GB in the exact runtime | Feasibility and all later paid work | Measure one A40/A6000 run; allow one A100 80 GB rerun only for insufficient VRAM. |
...
| A40/A6000 Secure Cloud capacity may be unavailable | Feasibility/live flow | Fail visibly; do not choose a smaller GPU. Retry later or use the one allowed A100 feasibility path only for VRAM, not capacity laundering. |
