...
from vla_pick_and_place.config import EE_SITE, IMG_H, IMG_W, N_JOINTS, SUCCESS_SETTLE_STEPS, TASK
from vla_pick_and_place.env.ik import solve_ik
...
def test_cube_spawns_inside_the_tight_region(env):
    ...
    from vla_pick_and_place.config import CUBE_SPAWN_CENTER, CUBE_SPAWN_HALF_EXTENT
...
def test_truncates_at_max_steps(env):
    from vla_pick_and_place.config import MAX_EPISODE_STEPS
...
def test_fast_moving_cube_through_the_zone_never_registers_success(env):
    ...
    env.reset(seed=0)
    from vla_pick_and_place.config import SUCCESS_MAX_SPEED
