...
from vla_pick_and_place.policy.evaluate import _to_batch
from vla_pick_and_place.policy.resolve import resolve_checkpoint
...
class EpisodeRunner:
    ...
    def __init__(self, device: str = INFERENCE_DEVICE, checkpoint: str = DEMO_CHECKPOINT):
        ...
        self.device = device
        # Resolve before the load so an unusable checkpoint fails during
        # gateway startup — where the log says why — instead of stranding a
        # visitor who already clicked Run.
        self.checkpoint = checkpoint = resolve_checkpoint(str(checkpoint))
    ...
    def _run_locked(self, controller: str, instruction: str, rec: rr.RecordingStream):
        from vla_pick_and_place.oracle.scripted_pick import OraclePolicy
