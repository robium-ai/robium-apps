...
**Goal:** Deliver a reusable `.foxe` Robot Control panel, integrate it as robot-navigation's right-side mapping control rail, and preinstall it in the bundled viewer.

**Architecture:** A shared TypeScript/React extension contains pure configuration, map parsing, drive-state, and adapter logic around `PanelExtensionContext`; robot-navigation supplies ROS defaults through layout state. The Docker build packages that shared source, copies the `.foxe` into the viewer, and runs a tested browser bootstrap before Lichtblick starts so a clean profile already contains the extension. The existing `teleop_relay` and `map_manager` remain the robot-side boundary.
...
### Task 3: Integrate the panel into robot-navigation

**Files:**
- Test: `robot-navigation/tests/test_lichtblick_control_layout.py`
...
```bash
git add robot-navigation
git commit -m "feat(robot-navigation): dock reusable Lichtblick controls"
```
...
- Modify: `REGISTRY.md`
- Modify: `learnings/2026-08-12-robot-navigation.md` in the sibling robium repo as events occur
...
```bash
git add shared/lichtblick-robot-control robot-navigation REGISTRY.md
git commit -m "docs(robot-navigation): document Lichtblick control extension"
```

### Task 5: Make the extension zero-install in robot-navigation
...
- Test: `robot-navigation/tests/test_lichtblick_bundle.py`
- Create: `robot-navigation/scripts/bundle_default_extension.py`
...
- Produces: a repository-root Docker build context so the robot-navigation image can package `shared/lichtblick-robot-control` without duplicating it.
...
Run: `python3 -m unittest robot-navigation.tests.test_lichtblick_bundle -v`
...
Run: `make control-extension-check` and the dependency-free robot-navigation unittest suite.
