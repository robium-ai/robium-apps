...
**Architecture:** The shared `.foxe` owns the reusable dashboard UI and ROS interaction mechanics. Each app's committed Lichtblick layout enables only the capabilities it needs and supplies its ROS interfaces and simulation choices; robot-navigation enables the complete navigation dashboard.
...
- Preserve the untracked saved maps under `robot-navigation/src/robot_nav_bringup/maps/`.
...
- [x] Preserve migration of existing version-3 robot-navigation state as a full dashboard.

### Task 3: Configure robot-navigation as a full dashboard consumer
...
- Modify: `robot-navigation/scripts/bundle_default_extension.py`

**Interfaces:**
- Consumes: `Robium Dashboard.dashboard` and config version 4.
- Produces: the existing full robot-navigation UI with House and Warehouse choices.

- [x] Enable all robot-navigation dashboard capabilities in its layout.
...
- [x] Update robot-navigation and registry references to Dashboard.
