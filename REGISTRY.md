...
| [vla-pick-and-place](vla-pick-and-place/) | Language-conditioned VLA (experimental) | pinned LeRobot v0.4.4 + official Pi0.5 LIBERO checkpoint | LIBERO-Goal task 8 (Franka Panda) | uv + Python 3.10; fake local fixtures; pinned Linux/amd64 CUDA image for RunPod | capability-scoped Gradio simulator frames | Doctor, 27 tests, fake rollout, protected CPU lifecycle, diagnostics, and closed-stdin LIBERO import passed 2026-08-24; paid revalidation confirmed the prompt fix but dependency-installed `hf-libero` shadowed the pinned checkout and lacked scene assets, so authoritative pinned imports and another paid gate remain pending |
