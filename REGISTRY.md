...
| [diffusion-policy-pusht](diffusion-policy-pusht/) | Physical AI / ML manipulation (experimental) | LeRobot 0.6.0 + pinned official 175k Diffusion Policy | gym-pusht + T benchmark / L-I-Z OOD probes | uv + Python 3.12 + native Apple MPS; no training required | Direct Gradio frames + additive Rerun timeline | `make smoke` (2) + `make demo-smoke` (3), all passed 2026-08-22; published reference: 65.4% / 500 episodes |
| [vla-pick-and-place](vla-pick-and-place/) | Language-conditioned VLA (experimental) | LeRobot 0.6.0 + SmolVLA 450M | MuJoCo 3.10 (SO-101) | uv + Python 3.12 (MPS) + HF Jobs (GPU train) | Robium workspace + embedded Rerun | `./app doctor` + `make test` (62) + `make smoke` + `make demo-smoke` (2026-08-17) |
