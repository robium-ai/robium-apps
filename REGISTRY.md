...
| [vla-pick-and-place](vla-pick-and-place/) | Language-conditioned VLA (experimental) | pinned LeRobot v0.4.4 + official Pi0.5 LIBERO checkpoint | LIBERO-Goal task 8 (Franka Panda) | uv + Python 3.10; fake local fixtures; pinned Linux/amd64 CUDA image for RunPod | capability-scoped Gradio simulator frames; eager interactive policy profile | Doctor, 41 tests, fake rollout, publication-pointer validation, duplicate-click busy handling, and rebuilt CPU container lifecycle pass; immutable digest `sha256:0bc3bae5…f083` completed one successful 75-step real-model episode with default compilation and passed proxy isolation/cancellation on RTX PRO 4500 (2026-08-24); deployed to the disabled private template; public 20-episode evaluation unclaimed; production disabled |
