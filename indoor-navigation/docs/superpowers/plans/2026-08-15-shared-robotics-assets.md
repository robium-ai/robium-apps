...
**Goal:** Add a repository-wide asset catalog and make indoor-navigation consume its two simulation worlds through stable shared asset IDs.
...
- Do not copy, move, delete, stage, or commit any existing file under `indoor-navigation/src/indoor_nav_bringup/maps/`.
...
### Task 3: Consume shared worlds from indoor-navigation

**Files:**
- Modify: `indoor-navigation/robium-app.yaml`
...
- Delete: `indoor-navigation/scripts/fetch_aws_small_house.py`
...
```bash
python3 -m py_compile indoor-navigation/src/indoor_nav_bringup/launch/sim.launch.py
python3 -c 'import yaml; data=yaml.safe_load(open("indoor-navigation/robium-app.yaml")); assert data["assets"]["worlds"] == ["world.aws-small-house", "world.tugbot-warehouse"]; print("APP ASSETS OK")'
git status --short -- indoor-navigation/src/indoor_nav_bringup/maps
```
...
```bash
git add indoor-navigation/robium-app.yaml indoor-navigation/docker/Dockerfile indoor-navigation/docker/Dockerfile.dockerignore indoor-navigation/src/indoor_nav_bringup/launch/sim.launch.py indoor-navigation/scripts/fetch_aws_small_house.py
git commit -m "refactor(indoor-navigation): consume shared world assets"
```

### Task 4: Establish the future writable-map boundary

**Files:**
- Create: `indoor-navigation/data/maps/.gitignore`
...
Create `indoor-navigation/data/maps/.gitignore` with:
...
- current maps still written beneath `src/indoor_nav_bringup/maps/`;
...
world assets to the indoor-navigation registry description. Do not claim a
...
```bash
rg -n "shared/assets|world.aws-small-house|world.tugbot-warehouse|data/maps|CC BY-NC-ND" indoor-navigation/README.md indoor-navigation/docs/architecture-brief.md REGISTRY.md
git status --short -- indoor-navigation/src/indoor_nav_bringup/maps
git diff --check
```
...
```bash
git add indoor-navigation/data/maps/.gitignore indoor-navigation/README.md indoor-navigation/docs/architecture-brief.md REGISTRY.md
git commit -m "docs(indoor-navigation): define shared asset boundaries"
```
...
- Inspect: `indoor-navigation/robium-app.yaml`
- Inspect: `indoor-navigation/docker/Dockerfile`
- Inspect: `indoor-navigation/src/indoor_nav_bringup/launch/sim.launch.py`
