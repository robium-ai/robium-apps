...
The first consumer is indoor-navigation. The design must preserve its pinned
...
- Do not move or delete the existing untracked indoor-navigation maps without
...
`indoor-navigation/data/maps/<world-id>/` rather than the ROS package source
tree. That directory is gitignored and mounted into the container so maps
survive container recreation.

The current untracked directories under
`indoor-navigation/src/indoor_nav_bringup/maps/` are migration inputs only.
...
The initial indoor-navigation declaration leaves `maps` empty. A canonical
...
indoor-navigation: immutable GitHub archives and pinned Gazebo Fuel resources.
...
- A manual indoor-navigation runtime check that both House and Warehouse load.
...
3. Declare those world IDs in indoor-navigation's `robium-app.yaml`.
4. Replace indoor-navigation's app-owned AWS fetch script and special asset
   path with the shared resolver.
5. Introduce and document the gitignored writable
   `indoor-navigation/data/maps/` boundary without switching the current
   runtime mount.
