...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/diffusion-policy-pusht-app.XXXXXX")
RUN_WRAPPER_PID=""
...
printf '%s\n' "$*" >>"$UV_CALLS"
case "$*" in
  "run python -m diffusion_policy_pusht.demo.app")
    echo "DEMO READY"
...
exit 0
EOF
chmod +x "$TEST_DIR/bin/uv" "$TEST_DIR/bin/ffmpeg"
...
assert_contains "$build_calls" "run python -m diffusion_policy_pusht.run prepare-official"
...
assert_contains "$status_output" "PushT with Diffusion Policy: RUNNING"
assert_contains "$status_output" "http://localhost:48765"
...
assert_contains "$stopped_output" "PushT with Diffusion Policy: STOPPED"

run_calls=$(cat "$UV_CALLS")
assert_contains "$run_calls" "run python -m diffusion_policy_pusht.demo.app"
