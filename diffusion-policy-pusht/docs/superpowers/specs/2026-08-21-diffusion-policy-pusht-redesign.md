...
## 2026-08-22 approved checkpoint pivot

The implementation no longer requires a locally trained checkpoint ladder.
The default is the pinned official `lerobot/diffusion_pusht` 175k checkpoint,
whose model card supplies a 500-episode, 100-denoise reference result. The app
adds a 10-denoise interactive mode, native MPS inference, randomized T/L/I/Z
layouts, and direct frames plus Rerun. The incomplete local 5k experiment may
appear only as separate evidence; it is not an earlier rung of the official
model. This approved pivot supersedes ladder-training requirements below
where they conflict, while preserving the evidence-first and honest-labeling
requirements.
