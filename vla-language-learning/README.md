...
```bash
make check     # preflight: uv present, lockfile intact, assets/venv hints
make sync      # uv sync
make assets    # vendor the MuJoCo menagerie SO-101 model (sparse checkout)
make oracle    # scripted IK pick, 10 episodes — the "is it alive" canary
export MUJOCO_GL=cgl   # macOS; osmesa/egl are Linux-only
make test      # the regression suite (54 tests, no Hub access needed)
```
...
Each stage is a `Makefile` target wrapping `python -m vla_language_learning.run <cmd>`;
`src/vla_language_learning/config.py` is the single source of truth for every
...
| Demo (native, MPS) | `make demo` | the demo-page gateway on :8765 — FastAPI session contract + Gradio/`gradio_rerun` UI (`src/vla_language_learning/demo/`); pair with robium-website's `/demos/vla-trial/` page in "direct" mode |
| Demo (container) | `make demo-image` | build `vla-language-learning:latest` (CPU/osmesa; checkpoint baked in via a BuildKit HF-token secret) — what the website's orchestrator spawns |
...
- The **entire remote training loop** is real and has run to completion on
  ...
  loads is a genuine artifact of that loop, not a placeholder.
...
- **macOS MuJoCo GL contexts (CGL) are thread-affine — cross-thread rendering
  ...
  thread (see `src/vla_language_learning/demo/episode_runner.py`'s docstring).
...
- **Camera renaming is required at both train AND eval.** SmolVLA's base
  ...
  `src/vla_language_learning/policy/evaluate.py`'s module docstring).
