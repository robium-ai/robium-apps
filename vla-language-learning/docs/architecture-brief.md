# vla-language-learning — architecture brief
...
| Second remote submit | 100 | a10g-small | **Full loop green.** Checkpoint pushed to `robium/train_2026-07-15_08-09-36` (not `POLICY_REPO_ID` — see Battle scars #8). Eval: 10 seeded episodes, **0/10 = 0% success** on MPS locally. |
...
8. **HF Jobs ignores `--policy.repo_id` for the final push.** The trained
   model landed at an auto-generated `robium/train_2026-07-15_08-09-36`
...
11. **The `.gitignore` negation pattern around `scene_pick.xml` is a
    ...
    (`src/vla_language_learning/env/assets/*`) and re-fetched by `make assets`, but
    ...
    an explicit negation line (`!src/vla_language_learning/env/assets/scene_pick.xml`)
