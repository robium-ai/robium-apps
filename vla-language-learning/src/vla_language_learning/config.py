"""Single source of truth for vla-language-learning run parameters.
...
# 2026-08-03: the original artifact lived under a personal account the org
# token cannot read; the dataset has been re-recorded and re-pushed to
# robium/so101_pick_cube, but re-minting this checkpoint (`make train`,
# ~$1-2; HF Jobs names it robium-admin/train_<timestamp> — read the "Model
# pushed to" job-log line) 402'd on missing prepaid Jobs credits. Until it
# is re-minted, point VLA_DEMO_CHECKPOINT at the local train-smoke artifact
# (SMOKE_CHECKPOINT_PATH) to run the demo/narrative paths.
NARRATIVE_FINETUNED_STANDIN = "robium/train_2026-07-15_08-09-36"
