"""CLI dispatcher: python -m vla_language_learning.run <subcommand>."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        print("usage: python -m vla_language_learning.run <subcommand>", file=sys.stderr)
        return 2
    ...
    if cmd == "spike-policy":
        from vla_language_learning.config import POLICY_SPIKE_N_PASSES
    ...
    if cmd == "viz-oracle":
        from vla_language_learning.config import VIZ_DIR
        from vla_language_learning.env.so101_pick import SO101PickEnv
        from vla_language_learning.oracle.scripted_pick import rollout
        from vla_language_learning.viz.rerun_logger import RerunLogger

        env = SO101PickEnv()
        logger = RerunLogger(app_id="vla_language_learning_oracle", save_path=VIZ_DIR / "oracle.rrd")
    ...
    if cmd == "push-dataset":
        ...
        from vla_language_learning.config import DATASET_REPO_ID
    ...
    if cmd == "train-smoke":
        ...
        from vla_language_learning.config import train_smoke_cmd
    ...
    if cmd == "train":
        ...
        from vla_language_learning.config import train_remote_cmd
    ...
    if cmd == "eval":
        from vla_language_learning.config import POLICY_REPO_ID, SMOKE_EVAL_EPISODES, SUCCESS_RATE_FLOOR
