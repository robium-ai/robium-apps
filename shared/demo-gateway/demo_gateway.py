#!/usr/bin/env python3
"""Robium demo session gateway — the reusable, vendorable reference copy.

Single process on $PORT (default 8765). Routes:
  * WebSocket upgrade (any path)  -> raw byte tunnel to the bridge at
      $BRIDGE_PORT. First tunnel claims the instance for the request's
      ?session=UUID; a second concurrent viewer gets 503 (the platform
      routes their retry to a fresh instance because this one is busy).
  * GET  /status?session=UUID     -> 200 JSON (contract in README.md);
      409 if the instance is claimed by a different session.
  * POST /shutdown?session=UUID   -> 200 + SIGINT to $SHUTDOWN_PID
      (default 1: the container's launch process); 403 on mismatch.
  * GET  /                        -> 302 to the bundled viewer with ds
      params targeting this host; other GETs serve $STATIC_ROOT.

stdlib only; runs alongside the app's launch process inside the demo
container. Apps VENDOR this file (copy it in); configuration is all env.
"""
import asyncio
import base64
import contextvars
...
BRIDGE = ('127.0.0.1', int(os.environ.get('BRIDGE_PORT', '8766')))
STATUS_PATH = os.environ.get('STATUS_PATH', '/tmp/demo_status.json')
SESSION_SECONDS = int(os.environ.get('SESSION_SECONDS', '1800'))
FLEET_BUDGET = int(os.environ.get('FLEET_BUDGET', '5'))  # sync with max instances
FLEET_CACHE_S = 30
WS_GUID = '258EAFA5-E914-47DA-95CA-C5AB0DC85B11'
# realpath: the traversal guard compares canonical paths, so a symlinked
# STATIC_ROOT (e.g. macOS /var -> /private/var tmpdirs) must be canonical too.
STATIC_ROOT = os.path.realpath(os.environ.get('STATIC_ROOT', '/opt/lichtblick'))
SHUTDOWN_PID = int(os.environ.get('SHUTDOWN_PID', '1'))
MIME = {
...
ALLOWED_ORIGINS = tuple(
    o.strip() for o in os.environ.get(
        'ALLOWED_ORIGINS', 'https://robium.ai,https://robium.org').split(',') if o.strip())
...
async def handle(reader, writer):
    ...
    if url.path == '/shutdown' and method == 'POST':
        ...
        # SIGINT, not SIGTERM: a launch process running as PID 1 typically
        # has no SIGTERM handler installed, and unhandled signals to PID 1
        # are ignored by the kernel — SIGINT triggers the real shutdown
        # (verified against ros2 launch).
        os.kill(SHUTDOWN_PID, signal.SIGINT)
        return
